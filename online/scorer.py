...
#         sys.exit(4)
def evalution(submit, ho_test_set):
    try:
        df_pred = submit#pd.read_csv(submitted_path)
    ...
    # Load target
    df_true = ho_test_set.sort_values(['ATM_ID', 'DATE']).reset_index(drop=True) #pd.read_csv(true_values_path).sort_values(['ATM_ID', 'DATE']).reset_index(drop=True)
    score = 10000-scorer(df_true, df_pred) * 10000
